...
public class StudentServiceDevTest {
    ...
    @Test
    public void testUpdateStudentPartialBeanWrapper() {
        Student student1 = studentService.createStudent(name1, "383 204 8903", Student.Status.HIBERNATING);

        assertEquals(5, studentService.getAllStudents().size());

        Map<String, Object> props = new HashMap<>();
        props.put("name", name2);
        Student.Status newStatus = Student.Status.PART_TIME;
        props.put("status", newStatus);


        studentService.updateStudentPartialBeanWrapper(student1.getId(), props);

        assertEquals(5, studentService.getAllStudents().size());
        Student uStudent = studentService.getStudent(student1.getId());
        assertEquals(name2, uStudent.getName());
        assertEquals(student1.getPhoneNumber(), uStudent.getPhoneNumber());
        //assertEquals(student1.getStatus(), uStudent.getStatus());
        assertEquals(newStatus, uStudent.getStatus());
    }
}